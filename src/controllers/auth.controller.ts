...
const JWT_SECRET = process.env.JWT_SECRET || "SportonAdh0Adm1n14A@";
...
export const signin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.error("Signin Eroor", error);
...
export const initiateAdmin = async (
  ...
  try {
    ...
    if (count > 0) {
      res.status(400).json({
        message:
          "We can only have 1 admin user, if you want to create a new admin user, please delete the user manually from database",
      });
      return;
    }
